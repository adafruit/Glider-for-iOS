                    ...
                    client.readFile(path: fileProviderItem.fullPath) { result in
                        ...
                        case .success(let data):
                            do {
                                ...
                                DLog("readFile \(fileProviderItem.fullPath) success")
                                completionHandler(nil)
                            }
                            catch(let error) {
                                DLog("readFile \(fileProviderItem.fullPath) write to disk error: \(error)")
                                completionHandler(error)
                            }
                            
                        case .failure(let error):
                            DLog("readFile \(fileProviderItem.fullPath) error: \(error)")
        ...
        if (containerItemIdentifier == NSFileProviderItemIdentifier.rootContainer) {
            ...
            if let item = try item(for: containerItemIdentifier) as? FileProviderItem  {
                if item.entry.isDirectory {
                    DLog("enumerator for directory: \(containerItemIdentifier)")
                    let path = item.path + item.entry.name + "/"
                    return FileProviderEnumerator(gliderClient: gliderClient, path: path  )
                }
                else {
                    DLog("TODO: enumerator for file: \(containerItemIdentifier)")
                }
            }
